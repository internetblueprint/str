...
def generate_nda_pdf(contract_data):
    ...
    # Other party details
    other_party_text = f"""(2) <b>{contract_data['other_name'].upper()}</b>{(", ID Number: " + contract_data['other_id']) if contract_data['other_id'] else ""},
    with address at {contract_data['other_address']} (hereinafter referred to as "the Recipient" or "Receiving Party")
    {( ", employed as " + contract_data['job_title'] + " from " + contract_data['employment_date'].strftime('%d %B %Y')) if contract_data['contract_type'] == 'Employee NDA' else ""}."""
...
def generate_nda_docx(contract_data):
    ...
    other_para.add_run(f"{(', ID Number: ' + contract_data['other_id']) if contract_data['other_id'] else ''}, with address at {contract_data['other_address']} (hereinafter referred to as \"the Recipient\" or \"Receiving Party\"){(', employed as ' + contract_data['job_title'] + ' from ' + contract_data['employment_date'].strftime('%d %B %Y')) if contract_data['contract_type'] == 'Employee NDA' else ''}.")
...
def add_remaining_clauses_to_story(story, contract_data, h1_style, h2_style, body_style):
    ...
    story.append(Paragraph("2.2 The Recipient shall use the Confidential Information solely for the purpose of performing their duties for the Company and shall not use it for any other purpose.", body_style))
    story.append(Paragraph("2.3 The Recipient shall take all reasonable steps to protect the confidentiality of the Confidential Information.", body_style))
...
def add_signature_section_to_story(story, contract_data, h1_style, body_style):
    ...
    story.append(Paragraph(f"{contract_data['company_rep']}\n{contract_data['company_position']}", body_style))
    story.append(Spacer(1, 20))

    # Witness
    witness_elements = [
        Paragraph("WITNESS:", body_style),
        Spacer(1, 30),
        Paragraph("_________________________________", body_style),
        Paragraph("Full Name:", body_style),
        Paragraph("Date:", body_style),
        Spacer(1, 20)
    ]
    story.append(KeepTogether(witness_elements))

    # Page break before recipient
    story.append(PageBreak())
    ...
    story.append(Spacer(1, 30))
    story.append(Paragraph("_________________________________", body_style))
    story.append(Paragraph(f"{contract_data['other_name']}\nID Number: {contract_data['other_id']}", body_style))
    story.append(Spacer(1, 20))
    ...
    story.append(KeepTogether(witness_elements2))
...
def generate_nda_text(contract_data):
    ...
    recipient_text = f"(2) {contract_data['other_name'].upper()}{( ', ID Number: ' + contract_data['other_id']) if contract_data['other_id'] else ''}, with address at {contract_data['other_address']} (hereinafter referred to as \"the Recipient\" or \"Receiving Party\"){( ', employed as ' + contract_data['job_title'] + ' from ' + contract_data['employment_date'].strftime('%d %B %Y')) if contract_data['contract_type'] == 'Employee NDA' else ''}."
...
WHEREAS, the Recipient {("is employed by" if contract_data['contract_type'] == "Employee NDA" else "will be engaged by")} the Company and will have access to such confidential information in the course of {"employment" if contract_data['contract_type'] == "Employee NDA" else "the engagement"};
...
{("ID Number: " + contract_data['other_id']) if contract_data['other_id'] else ''}
